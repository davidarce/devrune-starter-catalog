...
When user chooses "Commit": run the **Pre-Commit Gate** below, then invoke the `@git-commit` agent — do NOT run git commands directly.
When user chooses "Fix issues": delegate fixes to a sub-agent (orchestrator NEVER fixes code), then auto-launch review again.

### Pre-Commit Gate (run BEFORE invoking `@git-commit`)
...
5. If "Create new feature branch": run `git checkout -b feature/{change-name}` (or the slugified equivalent if the name has invalid chars). Then continue to `@git-commit`.
6. If branch is fit, skip the prompt and go straight to `@git-commit`.
